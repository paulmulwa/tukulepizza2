...
export default function PizzaDetailClient({
  slug,
}: {
  slug: string;
}) {
  ...
  useEffect(() => {
    ...
    const handlePop = () => setShowAR(false);
    window.history.pushState({ ar: true }, '', window.location.href);
  ...
  useEffect(() => {
    async function fetchPizza() {
      ...
      if (!isSupabaseConfigured || (typeof navigator !== 'undefined' && !navigator.onLine)) {
        if (!local) {
          setLoadError('This pizza is unavailable while offline.');
          setLoading(false);
        }
        return;
      }
      ...
      try {
        ...
        if (error || !data) {
          ...
          setPizza({
            ...
            ingredients: row.toppings || row.ingredients || [],
            tags: row.has_offer && row.offer_badge ? [row.offer_badge] : (row.tags || []),
            thumbnail_url: row.image_url || row.thumbnail_url || '',
            model_path: row.model3d_url || row.model_path || '',
            price_small: Number(row.price_small ?? 750),
            price_medium: Number(row.price_medium ?? 1000),
            price_large: Number(row.price_large ?? 2300),
            created_at: row.created_at || new Date().toISOString(),
          });
        }
      } catch {
        if (!local) setLoadError('Pizza is unavailable right now. Please try again shortly.');
      } finally {
        setLoading(false);
      }
    }
